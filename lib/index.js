...
async function updateFileTree() {
  fileTree.length = 0;
  fileTree = getGeneratedPaths(await getFolderContentRecursive(projectRoot + pageFolder), true).map((path) => path.slice(pageFolder.length));
  if (fileTree[0] === "")
    fileTree[0] = "/";
}
function getFileTree(includeAddLocation, includeUnpublished, refs = fileTree) {
...
import { z as z2, ZodDefault, ZodOptional } from "zod";
...
function useComponent(componentName, id, context = useComponentCtx) {
  const { addMmsg, addPageData: addPageData2, components: components2, dataFromPage, render, sendError } = context;
  const editorMode = isEditorMode();
  const { description, schema, template } = components2[componentName];
...
function render({ data, editorMode, pageSettings, component, id, template }) {
  ...
  if (editorMode) {
    const editorResult = id !== undefined ? result.replace(/data-editor="(.*?)"/g, (_2, p1) => {
      return `data-editor="${p1}.${id}"`;
    }) : result;
    return editorResult;
  }
...
function registerComponentForEditor(componentName, schema) {
...
async function partialPageUpdate({ component, data, editorData: editorData2, id, url }) {
  function reformatComponentData() {
    if (component === editorData2)
      return { [component]: $(data) };
    const [_component, editor, id2] = editorData2.split(".");
    return id2 === undefined ? { [component]: { [editor]: $(data) } } : { [component]: { [id2]: { [editor]: $(data) } } };
  }
  const { dataToLoad, templateToLoad } = await extractFromUrl(url);
  const isShared = id && id.startsWith(sharedIndex);
  const originalData = isShared ? await loadSharedData() : await loadPagesData(dataToLoad);
  ...
  pageData = merge(pageData, newData);
  messages.length = 0;
  const { template } = await import(templateToLoad);
  const page = await template();
  const pageDom = new JSDOM(page).window.document;
  const content = pageDom.querySelector("body").innerHTML;
  return {
...
var __dirname = "/home/lionel/Documents/dev/personal/yajsb/src/adapters/server";
var routesSite = {
  ...w(process.cwd() + "/site/public", "/"),
...
async function handleRouteSite(req) {
  return await H(req, {
    handler: async (request) => {
      ...
      if (!getFileTree(false, false).includes(url))
...
var constants = { componentFolder, dataExtension, index, pageFolder, projectRoot, templateExtension, templateFolder, tsExtension };
export {
  utils,
  useComponent,
  startYajsb,
