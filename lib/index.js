...
import { ZodDefault, ZodEnum, ZodObject, ZodOptional, z as z2 } from "zod";
...
function getSchemaAndType(componentDescription) {
...
function addExtra(zodEl, extra) {
  switch (extra) {
    ...
    case "url":
    case "urlPicker":
      zodEl = zodEl.url().describe(describeComponent({ wrapper: "urlPicker" }));
      break;
    case "html":
      break;
    default:
      zodEl = zodEl.describe(describeComponent({ wrapper: extra }));
      break;
  }
}
...
function useComponent(componentName, id, context = useComponentCtx) {
  const { addPageData: addPageData2, components: components2, dataFromPage, render } = context;
  const editorMode = isEditorMode();
  const { schema, template } = components2[componentName];
  let data = dataFromPage(componentName, id);
  if (!validateComponentSchemaAndTemplate(schema, template, componentName, id, data))
    return "";
...
function validateComponentSchemaAndTemplate(schema, template, componentName, id, data, sendError = errorComponent) {
...
function prepareComponentData(schema, data) {
  return schema !== null ? merge(getDefaultData(schema), data ?? {}) : data;
}
...
function component(isSingle, template, fields) {
