...
async function updateFileTree() {
  fileTree.length = 0;
  fileTree = getGeneratedPaths(await getFolderContentRecursive(projectRoot + pageFolder), true).map((path) => path.slice(pageFolder.length)).map((path) => path.endsWith(".index") ? path.split("/").slice(0, -1).join("/") + "/" : path);
  if (fileTree[0] === "")
    fileTree[0] = "/";
}
